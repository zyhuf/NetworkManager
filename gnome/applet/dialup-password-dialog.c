/* NetworkManager -- user and password utility for dialup connections
 *
...
child_stdout_data_cb (GIOChannel *source, GIOCondition condition, gpointer userdata)
{
	...
	GSList **user_pass = (GSList **) io_user_data->user_pass;
	...
	if (g_io_channel_read_line (source, &str, NULL, NULL, NULL) == G_IO_STATUS_NORMAL) {
		...
		if (len == 1 && str[0] == '\n') {
			...
			str[len - 1] = '\0';
			*user_pass = g_slist_append (*user_pass, str);
		}
	}
...
GSList *
nma_dialup_request_username_and_password (NMApplet *applet,
					  const char *name, const char *service,
					  gboolean retry)
{
	const char       *argv[] = {NULL       /* executable */, 
				    "-n", NULL /* dialup name */, 
				    "-s", NULL /* dialup service */, 
				    "-r",      /* retry */
			      NULL};
	GSList     *user_pass = NULL;
	int         child_stdin;
	int         child_stdout;
	GPid        child_pid;
	int         child_status;
	...
	if (auth_dialog_binary == NULL) {
		...
		dialog = gtk_message_dialog_new (NULL,
						 ...
						 GTK_MESSAGE_ERROR,
						 GTK_BUTTONS_CLOSE,
						 _("Cannot start dialup connection '%s'"),
						 name);
		gtk_message_dialog_format_secondary_text (GTK_MESSAGE_DIALOG (dialog),
  _("Could not find the authentication dialog for dialup connection type '%s'. Contact your system administrator."),
							  service);
	...
	if (!g_spawn_async_with_pipes (NULL,                       /* working_directory */
		...
		dialog = gtk_message_dialog_new (NULL,
						 ...
						 name);
		gtk_message_dialog_format_secondary_text (GTK_MESSAGE_DIALOG (dialog),
  _("There was a problem launching the authentication dialog for dialup connection type '%s'. Contact your system administrator."),
							  service);
		gtk_window_present (GTK_WINDOW (dialog));
		g_signal_connect_swapped (dialog, "response", G_CALLBACK (gtk_widget_destroy), dialog);
		goto out;
	...
	io_user_data.user_pass = &user_pass;
