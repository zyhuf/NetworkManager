                                          ...
                                          int *out_order)
{
	...
	res = abstract_info->meta_type->included_in_common (abstract_info, &order);
	 ...
	 *
	 * For infos that are included in "common" fields, their oder matters.
	 * Enfore that the callee set a non-negative sort order. */
	nm_assert ((!res && order == 0) || (res && order >= 0));
                                                  ...
                                                  gpointer *out_to_free)
{
	...
	for (i = 0; i < l; i++) {
		...
		included_in_common = nm_meta_abstract_info_included_in_common (abstract_infos[i], &order);
		...
		if (!infos_common) {
			infos_common = g_new (SelectIncludedInCommonData, l - i + 1);
			for (j = 0; j < i; j++) {
