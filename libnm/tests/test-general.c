...
static void
_do_read_vpn_details_impl (const char *file,
                           int line,
                           char *mem,
                           gsize len,
...
{
	nm_auto_close int memfd = -1;
	gssize written;
	off_t lseeked;
	gs_unref_hashtable GHashTable *data = NULL;
	gs_unref_hashtable GHashTable *secrets = NULL;

	memfd = memfd_create ("libnm-test-read-vpn-details", MFD_CLOEXEC);
	if (memfd < 0) {
...
#define _do_read_vpn_details_empty(str) \
	_do_read_vpn_details_impl (__FILE__, \
	                           __LINE__, \
	                           ""str"", \
	                           NM_STRLEN (str), \
	                           NULL, \
...
#define _do_read_vpn_details(str, expected_data, expected_secrets) \
	_do_read_vpn_details_impl (__FILE__, \
	                           __LINE__, \
	                           ""str"", \
	                           NM_STRLEN (str), \
