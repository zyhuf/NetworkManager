 ...
 **/
void
nm_tc_qdisc_set_parent (NMTCQdisc *qdisc, guint32 parent)
{
	g_return_if_fail (qdisc != NULL);
	g_return_if_fail (qdisc->refcount > 0);
 ...
 **/
void
nm_tc_qdisc_set_info (NMTCQdisc *qdisc, guint32 info)
{
	g_return_if_fail (qdisc != NULL);
	g_return_if_fail (qdisc->refcount > 0);
                  ...
                  GError **error)
{
	...
	action->kind = g_intern_string (kind);
...
nm_tc_action_dup (NMTCAction *action)
{
	...
	copy = nm_tc_action_new (action->kind,
	                         NULL);
 ...
 **/
void
nm_tc_action_set_kind (NMTCAction *action, const char *kind)
{
	g_return_if_fail (action != NULL);
	g_return_if_fail (action->refcount > 0);
...
nm_tc_tfilter_equal (NMTCTfilter *tfilter, NMTCTfilter *other)
{
	...
	if (   tfilter->family != other->family
	    || tfilter->handle != other->handle
	    || tfilter->parent != other->parent
	    || tfilter->info != other->info
...
nm_tc_tfilter_dup (NMTCTfilter *tfilter)
{
	...
	copy = nm_tc_tfilter_new (tfilter->kind, tfilter->family,
	                        tfilter->handle, tfilter->parent, tfilter->info,
	                        NULL);
 ...
 **/
void
nm_tc_tfilter_set_kind (NMTCTfilter *tfilter, const char *kind)
{
	g_return_if_fail (tfilter != NULL);
	g_return_if_fail (tfilter->refcount > 0);
 ...
 **/
int
nm_tc_tfilter_get_family (NMTCTfilter *tfilter)
{
	g_return_val_if_fail (tfilter != NULL, AF_UNSPEC);
	g_return_val_if_fail (tfilter->refcount > 0, AF_UNSPEC);

	return tfilter->family;
}

/**
 * nm_tc_tfilter_set_family:
 * @tfilter: the #NMTCTfilter
 * @family: address family (%AF_UNSPEC)
 *
 * Sets the address family associated with the
 ...
 **/
void
nm_tc_tfilter_set_family (NMTCTfilter *tfilter, int family)
{
	g_return_if_fail (tfilter != NULL);
	g_return_if_fail (tfilter->refcount > 0);
 ...
 **/
void
nm_tc_tfilter_set_parent (NMTCTfilter *tfilter, guint32 parent)
{
	g_return_if_fail (tfilter != NULL);
	g_return_if_fail (tfilter->refcount > 0);
 ...
 **/
guint32
nm_tc_tfilter_get_info (NMTCTfilter *tfilter)
{
	g_return_val_if_fail (tfilter != NULL, TC_H_UNSPEC);
	g_return_val_if_fail (tfilter->refcount > 0, TC_H_UNSPEC);
 ...
 **/
void
nm_tc_tfilter_set_info (NMTCTfilter *tfilter, guint32 info)
{
	g_return_if_fail (tfilter != NULL);
	g_return_if_fail (tfilter->refcount > 0);
...
nm_tc_tfilter_get_action (NMTCTfilter *tfilter)
{
	...
	nm_tc_action_ref (tfilter->action);
 ...
 **/
NMTCTfilter *
nm_setting_tc_config_get_tfilter (NMSettingTCConfig *self, int idx)
{
	g_return_val_if_fail (NM_IS_SETTING_TC_CONFIG (self), NULL);
	g_return_val_if_fail (idx >= 0 && idx < self->tfilters->len, NULL);
 ...
 **/
void
nm_setting_tc_config_remove_tfilter (NMSettingTCConfig *self, int idx)
{
	g_return_if_fail (NM_IS_SETTING_TC_CONFIG (self));

	g_return_if_fail (idx >= 0 && idx < self->tfilters->len);
...
_qdiscs_to_variant (GPtrArray *qdiscs)
{
	...
	if (qdiscs) {
		for (i = 0; i < qdiscs->len; i++) {
			...
			g_variant_builder_add (&qdisc_builder, "{sv}", "info",
...
_tfilters_to_variant (GPtrArray *tfilters)
{
	...
	if (tfilters) {
		for (i = 0; i < tfilters->len; i++) {
			...
			g_variant_builder_add (&tfilter_builder, "{sv}", "family",
			                       g_variant_new_uint32 (nm_tc_tfilter_get_family (tfilter)));
			...
			g_variant_builder_add (&tfilter_builder, "{sv}", "info",
			                       g_variant_new_uint32 (nm_tc_tfilter_get_info (tfilter)));
...
_tfilters_from_variant (GVariant *value)
{
	...
	while (g_variant_iter_next (&iter, "@a{sv}", &tfilter_var)) {
		...
		g_variant_lookup (tfilter_var, "family", "u", &family);
		g_variant_lookup (tfilter_var, "handle", "u", &handle);
		g_variant_lookup (tfilter_var, "parent", "u", &parent);
		g_variant_lookup (tfilter_var, "info", "u", &info);
		action_var = g_variant_lookup_value (tfilter_var, "action", G_VARIANT_TYPE_VARDICT);

		tfilter = nm_tc_tfilter_new (kind, family, handle, parent, info, &error);
		if (!tfilter) {
