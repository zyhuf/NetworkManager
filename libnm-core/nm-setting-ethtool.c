...
typedef gboolean (*gendata_filter_fcn)(const char *name);
...
static gboolean
_gendata_get_uint32 (NMSetting *setting,
                     const char *optname,
                     guint32 *out_value)
{
	GVariant *v;
...
static void
_gendata_set_uint32 (NMSetting *setting,
                     const char *optname,
                     guint32 value)
{
	nm_assert (NM_IS_SETTING (setting));
...
static gboolean
_gendata_clear (NMSetting *setting,
                const char *optname)
{
	GHashTable *ht;
...
static gboolean
_gendata_clear_all (NMSetting *setting,
                    gendata_filter_fcn filter)
{
	GHashTable *ht;
	const char *name;
	GHashTableIter iter;
                                 ...
                                 guint32 *out_value)
{
	...
	return _gendata_get_uint32 (NM_SETTING (setting), optname, out_value);
}
                                 ...
                                 guint32 value)
{
	...
	_gendata_set_uint32 (NM_SETTING (setting),
	                     optname,
	                     value);
	_notify_attributes (setting);
}
                                   ...
                                   const char *optname)
{
	...
	if (_gendata_clear (NM_SETTING (setting), optname))
		_notify_attributes (setting);
}
...
nm_setting_ethtool_clear_coalesce_all (NMSettingEthtool *setting)
{
	...
	if (_gendata_clear_all (NM_SETTING (setting), &nm_ethtool_optname_is_coalesce))
		_notify_attributes (setting);
}
...
nm_setting_ethtool_clear_ring_all (NMSettingEthtool *setting)
{
	...
	if (_gendata_clear_all (NM_SETTING (setting), &nm_ethtool_optname_is_ring))
		_notify_attributes (setting);
}
