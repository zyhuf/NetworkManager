...
/**
 * nm_setting_wireless_security_get_wps:
 * @setting: the #NMSettingWirelessSecurity
 *
 * Returns: the #NMSettingWirelessSecurity:wps property of the setting
 *
 * Since: 1.10
 **/
NM80211WpsFlags
nm_setting_wireless_security_get_wps (NMSettingWirelessSecurity *setting)
{
	g_return_val_if_fail (NM_IS_SETTING_WIRELESS_SECURITY (setting), NM_802_11_WPS_DISABLED);

	return NM_SETTING_WIRELESS_SECURITY_GET_PRIVATE (setting)->wps;
}
...
verify (NMSetting *setting, NMConnection *connection, GError **error)
{
	...
	/* WPS */
	if (priv->wps == NM_802_11_WPS_UNRECOGNIZED || priv->wps > NM_802_11_WPS_PIN) {
		g_set_error_literal (error,
		                     NM_CONNECTION_ERROR,
		                     NM_CONNECTION_ERROR_INVALID_PROPERTY,
		                     _("property is invalid"));
		g_prefix_error (error, "%s.%s: ", NM_SETTING_WIRELESS_SECURITY_SETTING_NAME, NM_SETTING_WIRELESS_SECURITY_WPS);
		return FALSE;
	}

	if (priv->wps & NM_802_11_WPS_DISABLED && priv->wps != NM_802_11_WPS_DISABLED) {
		g_set_error_literal (error,
		                     NM_CONNECTION_ERROR,
		                     NM_CONNECTION_ERROR_INVALID_PROPERTY,
              ...
              const GValue *value, GParamSpec *pspec)
{
	...
	case PROP_WEP_KEY_TYPE:
		...
		break;
	case PROP_WPS:
		priv->wps = g_value_get_int (value);
		break;
	case PROP_WPS_PIN:
		g_free (priv->wps_pin);
              ...
              GValue *value, GParamSpec *pspec)
{
	...
	case PROP_WEP_KEY_TYPE:
		...
		break;
	case PROP_WPS:
		g_value_set_int (value, priv->wps);
		break;
	case PROP_WPS_PIN:
	...
	g_object_class_install_property
		(object_class, PROP_WPS,
		 g_param_spec_int (NM_SETTING_WIRELESS_SECURITY_WPS, "", "",
		                   G_MININT, G_MAXINT, NM_802_11_WPS_AUTO,
		                   G_PARAM_READWRITE |
		                   G_PARAM_CONSTRUCT |
		                   NM_SETTING_PARAM_FUZZY_IGNORE |
		                   G_PARAM_STATIC_STRINGS));
