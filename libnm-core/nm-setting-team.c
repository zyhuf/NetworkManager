...
/**
 * nm_team_link_watcher_ethtool_new:
...
/**
 * nm_team_link_watcher_nsna_ping_new:
 ...
 **/
NMTeamLinkWatcher *
nm_team_link_watcher_nsna_ping_new (gint init_wait,
                                    gint interval,
                                    gint missed_max,
...
/**
 * nm_team_link_watcher_arp_ping_new:
 * @init_wait: init_wait value
 * @interval: interval value
 * @missed_max: missed_max value
 ...
 **/
NMTeamLinkWatcher *
nm_team_link_watcher_arp_ping_new (gint init_wait,
                                   gint interval,
                                   gint missed_max,
                                   const char *target_host,
                                   ...
                                   GError **error)
{
	...
	if (!target_host) {
		g_set_error (error, NM_CONNECTION_ERROR, NM_CONNECTION_ERROR_FAILED,
		             _("Missing target-host in arp_ping link watcher"));
		return NULL;
	}
	if (!source_host) {
		g_set_error (error, NM_CONNECTION_ERROR, NM_CONNECTION_ERROR_FAILED,
		             _("Missing source-host in arp_ping link watcher"));
		return NULL;
	}
	watcher = g_slice_new0 (NMTeamLinkWatcher);
	watcher->refcount = 1;
...
nm_team_link_watcher_dup (NMTeamLinkWatcher *watcher)
{
	...
	case LINK_WATCHER_ETHTOOL:
		return nm_team_link_watcher_ethtool_new (watcher->ethtool.delay_up,
		                                         watcher->ethtool.delay_down,
		                                         NULL);
		break;
	case LINK_WATCHER_NSNA_PING:
		return nm_team_link_watcher_nsna_ping_new (watcher->nsna_ping.init_wait,
	...
	case LINK_WATCHER_ARP_PING:
		return nm_team_link_watcher_arp_ping_new (watcher->arp_ping.init_wait,
