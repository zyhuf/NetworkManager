...
nm_setting_ip6_config_class_init (NMSettingIP6ConfigClass *ip6_class)
{
	 ...
	 * lease file belonging to the connection. If no DUID is found, a global and
	 ...
	 * try providing a stable DUID-LLT, the time field will contain a static time
	 * value that will be common to all the connections: the generation time of
	 * the host secret key used for the generation of stable addresses (see stable-id).
	 *
	 * The special values "stable-llt", "stable-ll" and "stable-uuid" will generate
	 * a DUID of the corresponding type, derived from the connection's stable-id.
	 ...
	 * of the interval is the same static value used in "llt").
	 *
