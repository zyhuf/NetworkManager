...
_nm_utils_team_link_watcher_from_json (json_t *json_element)
{
	...
	if (nm_streq0 (name, NM_TEAM_LINK_WATCHER_ETHTOOL))
		return nm_team_link_watcher_ethtool_new (val1, val2, NULL);
	else if (nm_streq0 (name, NM_TEAM_LINK_WATCHER_NSNA_PING))
		return nm_team_link_watcher_nsna_ping_new (val1, val2, val3, target_host, NULL);
	else if (nm_streq0 (name, NM_TEAM_LINK_WATCHER_ARP_PING)) {
		return nm_team_link_watcher_arp_ping_new (val1, val2, val3, target_host,
		...
		if (nm_streq (name, NM_TEAM_LINK_WATCHER_ETHTOOL)) {
			...
			if nm_streq (name, NM_TEAM_LINK_WATCHER_ARP_PING) {
				...
				if (g_variant_lookup (watcher_var, "send-always", "b"))
					flags |= NM_TEAM_LINK_WATCHER_ARP_PING_FLAG_SEND_ALWAYS;
				watcher = nm_team_link_watcher_arp_ping_new (val1, val2, val3,
				                                             target_host, source_host,
				                                             flags, &error);
			} else
				watcher = nm_team_link_watcher_nsna_ping_new (val1, val2, val3,
				                                              target_host, &error);
		}
		if (!watcher) {
